...
def to_jd(year, month, day, hour, minute, tz_str):
    # 时区偏移容错：pytz 时区数据库在某些精简部署环境下可能缺失/异常，
    # 中国全境统一东八区(UTC+8)，故优先用固定偏移，避免云端 pytz 反查失败导致 jd 异常。
    utc_offset_hours = 8.0  # Asia/Shanghai 默认
    try:
        tz = pytz.timezone(tz_str)
        local_dt = _localize_strict(tz, datetime(year, month, day, hour, minute))
        utc_dt = local_dt.astimezone(pytz.utc)
    except Exception:
        # 时区解析失败：回退到固定偏移（东八区/上海）
        utc_dt = datetime(year, month, day, hour, minute) - timedelta(hours=utc_offset_hours)
...
def calculate_full_chart(year, month, day, hour, minute, lat, lon,
    ...
    # 4. SAV/BAV (PyJHora — 云端环境可能因 pysweph 版本/时区差异失败，降级为空不影响核心排盘)
    tz = pytz.timezone(tz_str)
    ...
    try:
        ...
        ashtak = {'sarvashtakavarga': {}, 'bhinnashtakavarga': {}, 'sav_total': 0, 'sodhya_pindas': None}
    ...
    # 5. Divisional charts (PyJHora: 15 charts) — 降级为空 dict
    try:
        divisional_charts = _div_pyjhora(
            year, month, day, hour, minute, lat, lon, _tz_offset
        ) if _div_pyjhora is not None else {}
    except Exception:
        divisional_charts = {}
    ...
    # 11. Vimsottari Dasha MD/AD/PD (PyJHora — 降级为空列表)
    try:
        dashas = _dasha_pyjhora(year, month, day, hour, minute, lat, lon, _tz_offset)
    except Exception:
        dashas = []
    
    # 12. Shadbala (PyJHora — 降级为空)
    try:
